...
% -------------------------------------------------------------------------
    ...
    methods(Access = private)
        ...
        function [data, hppcPulseSequence] = getPulsesFromHPPCdata(obj)
            ...
            % Cycle to modify the current array, replacing zero to each 
            % positive current value to avoid problems for the discharge
            % pulses identification

            for i = 1:length(current_dischg)
                if current_dischg(i) > 0
                    current_dischg(i) = 0;
                else
                    current_dischg(i);
                end
            ...
            % Checking if the switching operation has been successful: n.
            % of positive elements has to match the n. of zeros

            % length(find(obj.InputTestData.I > 0));
...
%                 disp(['DEBUG: the indxDischgPulseEnd indexes are: ', num2str(indxDischgPulseEnd)])
            ...
            % Cycle to modify the current array, replacing zero to each 
            % negative current value to avoid problems for the charge
            % pulses identification

            for i = 1:length(current_chg)
                if current_chg(i) < 0
                    current_chg(i) = 0;
                else
                    current_chg(i);
            ...
            % Checking if the switching operation has been successful: n.
            % of negative elements has to match the n. of zeros

            % length(find(obj.InputTestData.I < 0))
            % length(find(current_chg == 0))
            % current_chg;
