...
% Rescale to have 37000 elements in the datasets
% Size = 37000;
% % HPPC40C
% HPPC40C.meas.Time = HPPC40C.meas.Time(1:Size);
% HPPC40C.meas.Voltage = HPPC40C.meas.Voltage(1:Size);
% HPPC40C.meas.Current = HPPC40C.meas.Current(1:Size);
% % HPPC25C
% HPPC25C.meas.Time = HPPC25C.meas.Time(1:Size);
% HPPC25C.meas.Voltage = HPPC25C.meas.Voltage(1:Size);
% HPPC25C.meas.Current = HPPC25C.meas.Current(1:Size);
...
ParsListDyn = {'Rp1', 'tau1', 'Rp2', 'tau2'};
InitGuessDyn = [0.006, 200, 0.006, 200];
ParsListTemp = {'Vnom_T2', 'R1_T2', 'V1_T2', 'Rp1_T2', 'tau1_T2', 'Rp2_T2', 'tau2_T2'};
InitGuessTemp = [3.8, 0.055, 3.6, 0.006, 200, 0.006, 200 ];
...
% Da capire ee_battery_lse che funzione è
...
OptParsMainDyn = [OptPars(1) OptParsDyn(1) OptPars(3:4) InitGuessMain(5) OptParsDyn(2:end)];
...
InitGuessTemp = [OptParsMainDyn(1:2) OptParsMainDyn(4) OptParsMainDyn(6:end)];
Pars = reshape([ParsListTemp; cellstr(num2str(InitGuessTemp'))'],1,[]);
for k=1:2:length(Pars)
    evalin('base',[Pars{k} '=' Pars{k+1} ';'])
end
...
% Optimize Battery charge dynamics parameters
assignin('base','ParsList', [ParsListTemp(2) ParsListTemp(4:end)]);
InitGuessTempDyn = [OptParsTemp(2) InitGuessTemp(4:end)];
OptParsTempDyn = fminsearch(@ee_battery_lse, InitGuessTempDyn, ...
                            optimset('TolX', 1e-3));

% Update Battery block with optimized parameters
OptParsTempDyn = [OptParsTemp(1) OptParsTempDyn(1) OptParsTemp(3) OptParsTempDyn(2:end)];
