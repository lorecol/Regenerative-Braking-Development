...
for cycle = 1:1/disCapStep
    ...
    power_supply.CCmode(Vlimreal,curr_discharge_pulse);
    pause(0.5); 
    % turn the output on
    power_supply.turnON;   
    tic
    ...
    % Charge 0.75C for 10s
    power_supply.CCmode(Vlimreal,curr_charge_pulse);
    pause(0.5); 
    % turn the output on
    power_supply.turnON;     
    tic
...
% Save the variable to the .mat file with the date-appended filename
save(fullfile('output/HPPC_Test',[sprintf('HPPC_data_%s', currentDateStr),'.mat'] ), 'current', 'voltage')
